package com.GauPass.constants;

import java.awt.Color;

public class UI_color {
    public static final Color DEEP_LILAC = new Color(163, 73, 164);
    public static final Color FOG = new Color(200, 191, 231);
    public static final Color BLACK = new Color(0, 0, 0);
    public static final Color WHITE = new Color(255, 255, 255);
    public static final Color RED = new Color(255, 0, 0);
    public static final Color AMARANTH = new Color(241, 72, 82);
    public static final Color VENETIAN_RED = new Color(188, 14, 22);
    public static final Color PALATINATE_PURPLE = new Color(112, 41, 99);
    public static final Color ELECTRIC_BLUE = new Color(125, 249, 255);
    public static final Color MAUVE = new Color(212, 156, 253);
    public static final Color ORANGE_YELLOW = new Color(255, 255, 0);
    public static final Color GREEN = new Color(0, 255, 0);
}